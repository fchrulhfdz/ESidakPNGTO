...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-600 mt-1">Kelola data umum & keuangan dan capaian kinerja</p>
        </div>
                                ...
                                <dl class="space-y-3">
                                    <div>
                                        <dt class="text-xs text-gray-500 font-medium">
                                            {{ $item->label_input_1 ?? 'Jumlah Proses Umum & Keuangan' }}
                                        </dt>
                                        <dd class="text-sm font-medium text-gray-900 mt-1">{{ $item->input_1 }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-xs text-gray-500 font-medium">
                                            {{ $item->label_input_2 ?? 'Jumlah Proses Tepat Waktu' }}
                                        </dt>
    ...
    <div id="inputContent" class="tab-content hidden">
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <form id="formInputData" action="{{ route('store.umum-keuangan') }}" method="POST">
                @csrf
                <input type="hidden" name="jenis" value="umum-keuangan">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2" id="label_input_1">
                                {{ $data->first()->label_input_1 ?? 'Jumlah Proses Umum & Keuangan' }}
                            </label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2" id="label_input_2">
                                {{ $data->first()->label_input_2 ?? 'Jumlah Proses Tepat Waktu' }}
                            </label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="label_input_1" required
                               class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white"
                               value="{{ old('label_input_1', 'Jumlah Proses Umum & Keuangan') }}"
                               placeholder="Contoh: Jumlah Proses Umum & Keuangan">
                        @error('label_input_1')
                            <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="label_input_2" required
                               class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white"
                               value="{{ old('label_input_2', 'Jumlah Proses Tepat Waktu') }}"
                               placeholder="Contoh: Jumlah Proses Tepat Waktu">
                        @error('label_input_2')
                            <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.edit-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            document.getElementById('editForm').action = `{{ url('umum-keuangan') }}/${id}`;
    ...
    function resetFormFields() {
        ...
        if (labelInput1) labelInput1.textContent = 'Jumlah Proses Umum & Keuangan';
        if (labelInput2) labelInput2.textContent = 'Jumlah Proses Tepat Waktu';
    ...
    if (hitungBtn) {
        hitungBtn.addEventListener('click', function() {
            const kegiatanUmumKeuangan = parseFloat(input1.value) || 0;
            const kegiatanTepatWaktu = parseFloat(input2.value) || 0;
            const target = parseFloat(targetHidden.value) || 0;
            
            if (kegiatanUmumKeuangan === 0) {
                alert('Jumlah proses umum & keuangan tidak boleh 0');
                return;
            }
            ...
            const realisasi = (kegiatanTepatWaktu / kegiatanUmumKeuangan) * 100;
    ...
    if (formInputData) {
        formInputData.addEventListener('submit', function(e) {
            ...
            if (!input1.value || !input2.value) {
                e.preventDefault();
                alert('Data input proses harus diisi');
                return;
            }

<div class="bg-white rounded-2xl border border-gray-200 p-6">
    <h2 class="text-lg font-semibold text-gray-900 mb-6">Input Data Perhitungan (Dua Input)</h2>
    
    <form id="formInputData1" action="{{ route('store.perkara') }}" method="POST">
        @csrf
        <input type="hidden" name="jenis" value="tipikor">
        <input type="hidden" name="tipe_input" value="dua_input">
        <input type="hidden" name="sasaran_strategis" id="sasaran_hidden_1" value="{{ old('sasaran_strategis') }}">
        <input type="hidden" name="indikator_kinerja" id="indikator_hidden_1" value="{{ old('indikator_kinerja') }}">
        <input type="hidden" name="target" id="target_hidden_1" value="{{ old('target') }}">
        <input type="hidden" name="rumus" id="rumus_hidden_1" value="{{ old('rumus') }}">
        <input type="hidden" name="bulan" id="bulan_hidden_1" value="{{ old('bulan') }}">
        <input type="hidden" name="tahun" id="tahun_hidden_1" value="{{ old('tahun') }}">
        <input type="hidden" name="label_input_1" id="label_input_1_hidden_1" value="{{ old('label_input_1') }}">
        <input type="hidden" name="label_input_2" id="label_input_2_hidden_1" value="{{ old('label_input_2') }}">
        <input type="hidden" name="realisasi" id="realisasi_hidden_1" value="{{ old('realisasi') }}">
        <input type="hidden" name="capaian" id="capaian_hidden_1" value="{{ old('capaian') }}">
        <input type="hidden" name="status_capaian" id="status_capaian_hidden_1" value="{{ old('status_capaian') }}">

        <div class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Bulan</label>
                    <select name="bulan" id="bulan_1" required
                ...
                <div>
                    ...
                    <input type="number" name="tahun" id="tahun_1" required min="2000" max="2100"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white"
            ...
            <div>
                ...
                <input type="text" id="sasaran_display_1" readonly
            ...
            <!-- DROPDOWN INDIKATOR KINERJA - Hanya tampilkan yang tipe_input dua_input -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Indikator Kinerja</label>
                <select id="pilihIndikator_1" required
                        class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white">
                    <option value="">Pilih Indikator Kinerja (Dua Input)</option>
                    @foreach($sasaran_strategis_dua_input as $item)
                        <option value="{{ $item->id }}" 
                                ...
                                data-tipe-input="dua_input"
                                @if(old('indikator_kinerja') == $item->indikator_kinerja) selected @endif>
                            {{ Str::limit($item->indikator_kinerja, 70) }}
                        </option>
                    @endforeach
                </select>
                @if($sasaran_strategis_dua_input->isEmpty())
                    <p class="text-xs text-red-500 mt-1">Belum ada sasaran strategis untuk tipe input dua input. Silakan tambah di menu Sasaran Strategis (2 Input).</p>
                @endif
            </div>
            ...
            <div>
                ...
                <input type="text" id="target_display_1" readonly
                       class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-gray-50"
                       value="{{ old('target') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2" id="label_input_1_display_1">
                        Label input 1
                    </label>
                    <input type="number" name="input_1" id="input_1_1" required min="0"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white"
                           value="{{ old('input_1') }}">
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2" id="label_input_2_display_1">
                        Label input 2
                    </label>
                    <input type="number" name="input_2" id="input_2_1" required min="0"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white"
                           value="{{ old('input_2') }}">
                </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button type="button" id="hitungBtn1" 
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" id="realisasi_result_1" readonly
                    ...
                    <div>
                        ...
                        <input type="text" id="capaian_result_1" readonly
                               class="w-full px-3 py-2 border border-blue-300 rounded-lg bg-white text-gray-900 font-medium">
                    </div>
                    
                    <div>
                        <label class="block text-xs text-gray-500 font-medium mb-1">Status Capaian</label>
                        <input type="text" id="status_capaian_result_1" readonly
                ...
                <div class="mt-4">
                    <div class="flex justify-between text-xs text-gray-500 mb-1">
                        ...
                        <span id="progress_percent_1">0%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div id="progress_bar_1" class="h-2 rounded-full bg-blue-500" style="width: 0%"></div>
                    </div>
            ...
            <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <textarea name="hambatan" id="hambatan_1" rows="3"
                    ...
                    <div>
                        ...
                        <textarea name="rekomendasi" id="rekomendasi_1" rows="3"
                                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white resize-none"
                    ...
                    <div>
                        ...
                        <textarea name="tindak_lanjut" id="tindak_lanjut_1" rows="3"
                                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white resize-none"
                    ...
                    <div>
                        ...
                        <textarea name="keberhasilan" id="keberhasilan_1" rows="3"
                                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white resize-none"
...
<script>
// Setup untuk form input data dua input
function setupInputDataForm(formNumber) {
    ...
    const labelInput2Hidden = formNumber === '1' ? document.getElementById(`label_input_2_hidden_${formNumber}`) : null;
    ...
    const labelInput2Display = formNumber === '1' ? document.getElementById(`label_input_2_display_${formNumber}`) : null;
    ...
    const input2 = formNumber === '1' ? document.getElementById(`input_2_${formNumber}`) : null;
    ...
    function filterIndikatorKinerja(formNum) {
        ...
        options.forEach(option => {
            ...
            // Pastikan tipe input sesuai dengan form (1 = dua_input, 2 = satu_input)
            const expectedTipeInput = formNum === '1' ? 'dua_input' : 'satu_input';
    ...
    function fillFormFields(selectedOption, formNum) {
        if (selectedOption.value) {
            ...
            const labelInput2Value = formNum === '1' ? selectedOption.getAttribute('data-label-input-2') : '';
            ...
            if (labelInput2Display && labelInput2Value) {
                labelInput2Display.textContent = labelInput2Value;
            }
    ...
    function resetFormFields(formNum) {
        ...
        if (labelInput1Display) labelInput1Display.textContent = 'Label input 1';
        if (labelInput2Display) labelInput2Display.textContent = 'Label input 2';
    ...
    if (hitungBtn) {
        hitungBtn.addEventListener('click', async function() {
            ...
            const perkaraTepatWaktu = input2 ? parseFloat(input2.value) || 0 : null;
            ...
            // Untuk dua input (formNumber === '1')
            if (perkaraDiselesaikan === 0) {
                alert('Jumlah perkara diselesaikan tidak boleh 0');
                return;
            }
            
            if (!perkaraTepatWaktu && perkaraTepatWaktu !== 0) {
                alert('Harap isi data input 2');
                return;
            }
            ...
            // Update tampilan
            document.getElementById(`realisasi_result_${formNumber}`).value = realisasi.toFixed(2) + '%';
            ...
            realisasiHidden.value = realisasi;
    ...
    if (formInputData) {
        formInputData.addEventListener('submit', function(e) {
            ...
            if (!input1.value) {
                e.preventDefault();
                alert('Data input 1 harus diisi');
                return;
            }
            
            if (formNumber === '1' && !input2.value) {
                e.preventDefault();
                alert('Data input 2 harus diisi');
                return;
            }
...
// Inisialisasi form input data dua input
setupInputDataForm('1');
</script>
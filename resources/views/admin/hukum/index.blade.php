...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        ...
        @else
            <span class="bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg text-sm font-medium border border-emerald-100">Admin Hukum</span>
        @endif
    </div>

    <!-- Include Navigation -->
    @include('admin.hukum.partials.nav_hukum')
    ...
    <div id="sasaranContent1Input" class="tab-content hidden">
        @include('admin.hukum.partials.sasaranstrategis1input')
    </div>
    @endif

    <!-- Tab Content: Lampiran PDF -->
    <div id="lampiranContent" class="tab-content hidden">
        @include('admin.hukum.partials.lampiran')
    </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-black bg-opacity-40 overflow-y-auto h-full w-full hidden z-50 transition-opacity duration-300 flex items-center justify-center p-4">
        <div class="relative bg-white rounded-2xl border border-gray-200 w-full max-w-2xl transform transition-all duration-300 scale-95">
            <div class="p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4" id="editModalTitle">Edit Data Perkara Hukum</h3>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function editHitungFunction() {
            ...
            if (perkaraDiselesaikan === 0) {
                alert('Jumlah perkara Hukum diselesaikan tidak boleh 0');
                return;
            }
        ...
        document.querySelectorAll('.edit-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                document.getElementById('editForm').action = `/hukum/${id}`;
                
                // Set modal title
                document.getElementById('editModalTitle').textContent = userType === 'superadmin' 
                    ? 'Edit Data Perkara Hukum (Super Admin)' 
                    : 'Edit Data Perkara Hukum (Admin)';
        ...
        async function hitungCapaianSatuInput(input1, target) {
            try {
                const response = await fetch('/hukum/calculate-satu-input', {
                    method: 'POST',
                    headers: {
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    tableBody.innerHTML = data.map(lampiran => {
                        ...
                        const capaian = lampiran.hukum ? lampiran.hukum.capaian : null;
                        let capaianDisplay = '-';
                        let capaianClass = '';
                        ...
                        return `
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <a href="/hukum/lampiran/${lampiran.id}/download" 
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    ${lampiran.hukum ? lampiran.hukum.indikator_kinerja : '-'}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    ${lampiran.hukum ? lampiran.hukum.sasaran_strategis : '-'}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    ${lampiran.hukum ? lampiran.hukum.nama_bulan + ' ' + lampiran.hukum.tahun : '-'}
                                </td>
        ...
        document.getElementById('filterLampiranBtn')?.addEventListener('click', function() {
            ...
            const options = document.querySelectorAll('#lampiranHukumSelect option');
            options.forEach(option => {
                ...
                if (matchBulan && matchTahun && matchIndikator) {
                    ...
                    if (option.selected) {
                        ...
                        document.getElementById('lampiranHukumSelect').value = '';
        ...
        document.getElementById('lampiranHukumSelect')?.addEventListener('change', function() {

...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">Perkara Pengawasan Intern (PHI)</h1>
            <p class="text-gray-600 mt-1">Kelola data perkara PHI dan capaian kinerja</p>
        </div>
        ...
        @else
            <span class="bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg text-sm font-medium border border-emerald-100">Admin PHI</span>
        @endif
    </div>

    <!-- Include Navigation -->
    @include('admin.phi.partials.nav_phi')
    ...
    <div id="sasaranContent1Input" class="tab-content hidden">
        @include('admin.phi.partials.sasaranstrategis1input')
    </div>
    @endif

    <!-- Tab Content: Lampiran PDF -->
    <div id="lampiranContent" class="tab-content hidden">
        @include('admin.phi.partials.lampiran')
    </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-black bg-opacity-40 overflow-y-auto h-full w-full hidden z-50 transition-opacity duration-300 flex items-center justify-center p-4">
        <div class="relative bg-white rounded-2xl border border-gray-200 w-full max-w-2xl transform transition-all duration-300 scale-95">
            <div class="p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4" id="editModalTitle">Edit Data Perkara PHI</h3>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function editHitungFunction() {
            ...
            if (perkaraDiselesaikan === 0) {
                alert('Jumlah perkara PHI diselesaikan tidak boleh 0');
                return;
            }
        ...
        document.querySelectorAll('.edit-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                document.getElementById('editModalTitle').textContent = userType === 'superadmin' 
                    ? 'Edit Data Perkara PHI (Super Admin)' 
                    : 'Edit Data Perkara PHI (Admin)';
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    tableBody.innerHTML = data.map(lampiran => {
                        ...
                        const capaian = lampiran.phi ? lampiran.phi.capaian : null;
                        let capaianDisplay = '-';
                        let capaianClass = '';
                        ...
                        return `
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <a href="/phi/lampiran/${lampiran.id}/download" 
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    ${lampiran.phi ? lampiran.phi.indikator_kinerja : '-'}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    ${lampiran.phi ? lampiran.phi.sasaran_strategis : '-'}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    ${lampiran.phi ? lampiran.phi.nama_bulan + ' ' + lampiran.phi.tahun : '-'}
                                </td>
        ...
        document.getElementById('filterLampiranBtn')?.addEventListener('click', function() {
            ...
            const options = document.querySelectorAll('#lampiranPhiSelect option');
            options.forEach(option => {
                ...
                if (matchBulan && matchTahun && matchIndikator) {
                    ...
                    if (option.selected) {
                        ...
                        document.getElementById('lampiranPhiSelect').value = '';
        ...
        document.getElementById('lampiranPhiSelect')?.addEventListener('change', function() {
